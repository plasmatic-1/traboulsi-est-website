...
2. **Second branch address**: the exact street address for the second branch couldn't be pulled automatically from the Google Maps link — only its GPS coordinates. The map, directions button, and "Call Now" all work correctly (they use the coordinates), but the **address text shown on the page currently says "Address to be confirmed"** — update it with the real address in `lib/locations.ts` (the `address` field on the `second-branch` entry). Also double check the phone, email, and opening hours for that branch — they're currently copied from the main branch as a placeholder.
