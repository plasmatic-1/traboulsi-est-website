...
4. **Contact form**: wired to Formspree (form ID `meeyrjwq`) via `@formspree/react`. The original form ID (`mgojglkl`) was tied to an unknown Formspree account with no known login, so this is a brand-new account you control, currently notifying your own email. Switch the destination email to `info@traboulsiest.com` in Formspree's dashboard once that inbox is confirmed reachable — no code changes needed for that.
